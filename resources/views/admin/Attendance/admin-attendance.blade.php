            ...
            <div class="pageFilter mb-3">
                <div class="row">
                        ...
                        <form  method="get" action="">
                            ...
                            <div class="leftFilters">
                                ...
                                <label for="floatingSelect">Year</label>
                                     <select class="form-select month js-select2" id="floatingSelect" name="year" aria-label="Floating label select example">
                                    <option value="all" selected>Select year</option>
                                        @for ($select_year=2023; $select_year<=date('Y', strtotime('+4 year')); $select_year++)
                                        <option value="{{$select_year}}" {{  ($year == $select_year) ? 'selected' : ''}}  >{{$select_year}}</option>
                                        @endfor
                                    </select>
                                ...
                                <div class="col-s buttons attendance adminAttendanceBtns">
                                         <button type="submit" value="Submit" class="btn btn-search attendance"><img src="images/iconSearch.svg" /> Search here</button>
                                         <button type="button" value="reset" class="btn btn-search attendance" onclick="window.location='{{ url("employee-attendance") }}'">Reset</button>
                                    </div>
                                    <!-- <div class="col">
                    ...
                    <div class="col-xl-4 col-lg-5 col-md-4">
                        <div class="rightFilter attendance attendanceRight attendanceres">
                            <?php 
                            $role=Role::where('_id',Auth::user()->user_role)->first();
...
<!--                             </button> -->
					...
					<div class="col-xl-4 col-lg-5 col-md-4">
                        <div class="rightFilter attendance attendanceRight">
                                <a class="download-attendance" href="/download-attendance">
                                    <svg width="30" height="27" viewBox="0 0 30 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M0 3.375C0 1.51348 1.49479 0 3.33333 0H11.6667V6.75C11.6667 7.6834 12.4115 8.4375 13.3333 8.4375H20V15.1875H11.25C10.5573 15.1875 10 15.7518 10 16.4531C10 17.1545 10.5573 17.7188 11.25 17.7188H20V23.625C20 25.4865 18.5052 27 16.6667 27H3.33333C1.49479 27 0 25.4865 0 23.625V3.375ZM20 17.7188V15.1875H25.7344L23.7031 13.1309C23.2135 12.6352 23.2135 11.8336 23.7031 11.3432C24.1927 10.8527 24.9844 10.8475 25.4688 11.3432L29.6354 15.5619C30.125 16.0576 30.125 16.8592 29.6354 17.3496L25.4688 21.5684C24.9792 22.0641 24.1875 22.0641 23.7031 21.5684C23.2188 21.0727 23.2135 20.2711 23.7031 19.7807L25.7344 17.724H20V17.7188ZM20 6.75H13.3333V0L20 6.75Z" fill="#1F1F1F"/>
                                    </svg>
                                </a>
                            ...
                            <a type="button" class="addBtn attendance m-0" data-bs-toggle="modal" data-bs-target="#myModal" onclick="$('.error').html('')">
                            Mark Attendance
                            </a>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        <th>Employee</th>
<!--                                         <th><img src="{{asset ('images/saturday.png')}}" alt="" width="20px"></th> -->
<!--                                         <th><img src="{{asset ('images/sunday.png')}}" alt="" width="20px"></th> -->
                                        @for($d = 1; $d<= $days; $d++)
                                        <?php 
                                        $date_today=strtotime($year . '-' . $month . '-' . $d);
                                        ?>
                                        <?php if(date('w',$date_today) == 6){?>
                                         <th><img src="{{asset ('images/saturday.png')}}" alt="" width="24px"></th>
                                        <?php }elseif (date('w',$date_today)== 0){?>
                                        <th><img src="{{asset ('images/sunday.png')}}" alt="" width="24px"></th>
                                        <?php }else{
                                            ?>
                                            <th>{{$d}}</th>
...
<div class="modal fade commonModalNew" id="myModal" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5 ml-auto" id="myModalLabel">Employee Attendance</h1>
      ...
      <div class="modal-body">
           <form method="post" enctype="multipart/form-data" id="addAttendance">
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                <label>Date</label>
                                <input type="date" id="date" class="form-control"  max="<?= date('Y-m-d'); ?>" name="date">
 ...
 $(document).ready(function() {
  $('#updateAttendance').submit(function(e) {
            ...
            $.ajax({
                ...
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                success: function(data) {
                 window.location = "/employee-attendance?employee="+employeeId;
                },
            });
        });
});
